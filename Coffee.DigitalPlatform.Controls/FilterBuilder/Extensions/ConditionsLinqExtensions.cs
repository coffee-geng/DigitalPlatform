    ...
    public static class ConditionsLinqExtensions
    {
        public static Expression<Func<T, bool>> BuildLambda<T>(this ConditionTreeItem conditionTreeItem)
        {
            ...
            return expression is null
                ? throw new InvalidOperationException($"Cannot create expression from condition tree item type '{conditionTreeItem.GetType().Name}'") : Expression.Lambda<Func<T, bool>>(expression, parameterExpression);
        }
            ...
            ParameterExpression parameterExpression)
        {
            return conditionTreeItem switch
            {
                ...
                _ => throw new InvalidOperationException($"Cannot create expression from condition tree item type '{conditionTreeItem.GetType().Name}'")
            };
        }
                ...
                default:
                    {
                        ...
                        if (baseType is not null && baseType.IsGenericType && baseType.GetGenericTypeDefinition() == typeof(NumericExpression<>))
                        {
                            return dataTypeExpression.BuildNumericExpression(parameterExpression, propertyMetadata.Name);
                        }
            ...
            Expression valueExpression, Expression isNullExpression)
        {
            return condition switch
            {
                ...
                _ => throw new NotSupportedException(string.Format(LanguageHelper.GetString("FilterBuilder_Exception_Message_ConditionIsNotSupported_Pattern"), condition))
            };
        }
            ...
            ParameterExpression parameterExpression, string propertyName)
        {
            return expression.SelectedCondition switch
            {
                ...
                _ => throw new NotSupportedException(string.Format(LanguageHelper.GetString("FilterBuilder_Exception_Message_ConditionIsNotSupported_Pattern"), expression.SelectedCondition))
            };
        }
